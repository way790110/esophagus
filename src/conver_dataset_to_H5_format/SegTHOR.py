...
def load_nii_to_numpy(file_path):
    ...
    vol = np.rot90(vol, 2, axes=(0, 2))
    return vol


def main():
    root = pathlib.Path(r'dataset\SegTHOR\train')

    count = 0

    # setting path 
    for folder_path in root.glob("*"):
        image_path = folder_path / f'{folder_path.stem}.nii.gz'
        label_path = folder_path / 'GT.nii.gz' 
        save_path = pathlib.Path(*[r'dataset\h5\SegTHOR', folder_path.stem + '.h5'])
        save_path.parent.mkdir(exist_ok=True, parents=True)

        # loading image & mask
        image = load_nii_to_numpy(image_path) 
        mask = load_nii_to_numpy(label_path) 
        mask_esophagus = np.where(mask == 1, 1, 0)
        ...
        #     save_path = pathlib.Path(*[r'dataset\png\SegTHOR', folder_path.stem, f'{i}.png'])
        #     save_path.parent.mkdir(exist_ok=True, parents=True)
